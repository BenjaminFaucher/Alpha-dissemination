...
screening <- screening.all.sub31$screening.1
...
delaysbefore=compute.delai(metadata)
...
first.VOC <- rbind(df,first.VOC)
rownames(first.VOC) =first.VOCf$country


first.VOC$date.collection <- as.Date(first.VOC$date.collection)
first.VOC$date.submission <- as.Date(first.VOC$date.submission)
first.VOC$date.1st.sub <- as.Date(first.VOC$date.1st.sub)
